...
public class LayoutAnchor : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (parentRT == null)
        {
            Debug.LogError("This component requires a RectTransform Parent", gameObject);
        }
    }
    ...
    public Vector2 AnchorPosition(TextAnchor myAnchor, TextAnchor parentAnchor, Vector2 offset)
    {
        ...
        Vector2 anchorCentre = new Vector2(Mathf.Lerp(myRT.anchorMin.x, myRT.anchorMax.x, myRT.pivot.x),
            Mathf.Lerp(myRT.anchorMin.y, myRT.anchorMax.y, myRT.pivot.y));
        Vector2 myAnchorOffset =
            new Vector2(parentRT.rect.width * anchorCentre.x, parentRT.rect.height * anchorCentre.y);
