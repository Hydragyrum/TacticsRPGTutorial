...
{
    public static float Linear(float start, float end, float value) => (end - start) * value + start;

    public static float Spring(float start, float end, float value)
    {
        value = Mathf.Clamp01(value);
        value = (Mathf.Sin(value * Mathf.PI * (0.2f + 2.5f * value * value * value))
                 * Mathf.Pow(1.0f - value, 2.2f) + value)
    ...
    public static float EaseInOutElastic(float start, float end, float value)
    {
        ...
        if (Math.Abs((value /= d / 2) - 2) < float.Epsilon)
            return start + end;
